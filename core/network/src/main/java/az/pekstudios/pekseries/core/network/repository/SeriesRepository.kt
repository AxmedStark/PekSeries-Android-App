package az.pekstudios.pekseries.core.network.repository
...
class SeriesRepository @Inject constructor(
                ...
                async {
                    ...
                    if (mazeId != null) {
                        ...
                        Show(
                            id = dto.id.toString(),
                            title = dto.name,
                            episode = dto.first_air_date?.let { "Premiere: ${formatDateFull(it)}" }
                                ?: "TMDB",
                            time = dto.vote_average?.let {
                                String.format(
                                    Locale.US,
                                    "Rating: ★ %.1f",
                                    it
        ...
        return try {
            ...
            results.map { item ->
                Show(
                    ...
                    episode = item.show.premiered?.let { "Premiere: ${formatDateFull(it)}" }
                        ?: "TV Show",
                    time = item.show.rating?.average?.let {
                        String.format(
                            Locale.US,
                            "Rating: ★ %.1f",
                            it
    ...
    suspend fun getShowEpisodes(tvMazeId: String): List<Episode> {
        return try {
            val dtos = tvMazeApi.getShowEpisodes(tvMazeId)
            dtos.map { dto ->
                Episode(
                    id = dto.id.toString(),
                    name = dto.name,
    ...
    suspend fun getSubscribedShows(): List<Show> {
        ...
        return try {
            ...
            coroutineScope {
                ...
                }.awaitAll().filterNotNull() // Ждем все одновременно и убираем ошибки
            }
        } catch (e: Exception) { emptyList() }
    }
    ...
    suspend fun getUpcomingSubscribedEpisodes(): List<Show> {
        ...
        return try {
            ...
            // 🚀 УМНАЯ ПАРАЛЛЕЛЬНАЯ ЗАГРУЗКА БЕЗ DELAY(500)
            val shows = coroutineScope {
                // Разбиваем на пачки по 5 штук, чтобы не получить бан от TVMaze за спам
                showIds.chunked(5).flatMap { chunk ->
                    chunk.map { id ->
                        async {
                            try {
                    ...
                    }.awaitAll() // Ждем пачку из 5 штук и переходим к следующей
                }
            }
